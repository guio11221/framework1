  ...
  app.get('/noticia', async function (req, res) {
    ...
    if (!id) {
      return res.status(404).send(
        ...
        <br>
        <h5>Exemplo: <code>http://host:porta/noticia?id=1</code></h5>
        </center>
        <br><hr>
        <center>
        <a href="/noticias">
    ...
    var result = await pool.query('SELECT * FROM noticias WHERE id_noticia = ?', [id])

    console.log(result)
    res.render("noticias/noticia", { dados: result });
