// Declaração da função construtora NoticiasDAO que recebe a conexão com o banco de dados
 function NoticiasDAO(connection) {
  ...
  this.SalvarNoticia = function (noticia, imgLink, callback) {
    ...
    conn.query(`INSERT INTO noticias (titulo, noticia, img) values (?,?,?)`,[noticia.titulo, noticia.noticia, imgLink], callback);
  };
  ...
  this.UpDateNoticia = function (noticia, callback) {
    ...
    conn.query(`UPDATE noticias SET titulo = ?, noticia = ?  WHERE id_noticia = ?`,[noticia.titulo, noticia.noticia, noticia.id], callback);
  };
}
